...
import { driverRegisterSchema } from '@wasalni/schemas';
...
export async function POST(req: NextRequest) {
  const parsed = await parseJson(req, driverRegisterSchema);
  if (parsed.kind === 'error') return parsed.response;

  try {
    const auth = new AuthEndpoints(backend);
    const session = await auth.registerDriver(parsed.data);
